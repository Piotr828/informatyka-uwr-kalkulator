dconst THEMATIC_TAGS = ["RPiS", "IO", "PiPO", "ASK", "SO", "SK", "BD"];
...
document.addEventListener("DOMContentLoaded", () => {
 ...
 function initChoices(selectEl, opts){
  ...
  function createCourseRow() {
   ...
   tagsSel.innerHTML = "";
    ...
    qs(".remove-course", node).addEventListener("click", () => {
      const semester = node.closest(".semester");
      node.remove();
      // usuń semestr jeśli pusty
      if (qs(".courses", semester).children.length === 0) {
        semester.remove();
        renumberSemesters();
      }
      recalcECTS();
  ...
  function createSemester() {
    ...
    const box = qs(".courses", node);
    box.appendChild(createCourseRow());
    return node;
  }
  ...
  function addCourseToCurrent() {
    let lastSem = semestersEl.lastElementChild;
    if (!lastSem) {
      addSemester();
      lastSem = semestersEl.lastElementChild;
    }
    qs(".courses", lastSem).appendChild(createCourseRow());
    recalcECTS();
    renderChecklist();
    updateSaveMode();
  }
  ...
  function renumberSemesters() {
    Array.from(semestersEl.children).forEach((sem, idx) => {
      qs(".semester-title", sem).textContent = `Semestr ${idx + 1}`;
    });
  }
  ...
  function getAllCourses(){
    ...
    Array.from(semestersEl.children).forEach((sem, sidx) => {
      Array.from(qs(".courses", sem).children).forEach(row => {
        ...
        const types = Array.from(typeSel.selectedOptions).map(o => o.value);
        const tags = Array.from(tagsSel.selectedOptions).map(o => o.value);

        result.push({ subject, ects, types, tags, semester: sidx+1 });
      });
    });
    return result;
  }
  ...
  function recalcECTS() {
    ...
    Array.from(semestersEl.children).forEach(sem => {
      const sum = Array.from(qs(".courses", sem).children).reduce((acc, row) => {
        return acc + (Number(qs(".ects", row).value) || 0);
      }, 0);
      qs(".ects-sem", sem).textContent = sum;
      total += sum;
    });
  ...
  function computeStatus(){
    ...
    const sumWhere = (pred) => courses.reduce((a,c)=> a + (pred(c) ? c.ects : 0), 0);
    const countWhere = (pred) => courses.reduce((a,c)=> a + (pred(c) ? 1 : 0), 0);
   ...
   const thematicCovered = Object.fromEntries(
      THEMATIC_TAGS.map(tag => [tag, countWhere(c => c.tags.includes(tag)) > 0])
    );

    return {
      details: {
        sumI, sumIInz, sumKInz, sumOIKP, sumHS, sumOWI, sumE, sumPr, sumThematic,
        cntP, cntPS, cntAng, thematicCovered
      },
      checks: {
        I:        sumI    >= RULES.minI,
  ...
  function renderChecklist(){
    ...
    const rows = [
      ...
      { ok: checks.THEMIN,   text: `Przedmioty tematyczne (RPiS, IO, PiPO, ASK, SO, SK, BD) ≥ ${RULES.thematicMin} ECTS (masz ${details.sumThematic})` },
      { ok: checks.THEEACH,  text: `Każdy znacznik tematyczny obecny co najmniej raz: ${THEMATIC_TAGS.map(t=>`${t}:${details.thematicCovered[t]?'✓':'–'}`).join(' ')}` },
      { ok: checks.PS,       text: `Proseminarium (PS): ≥ ${RULES.needPS}` },
      { ok: checks.PRAKTYKI, text: `Praktyka zawodowa (PRAKTYKI) ≥ ${RULES.minPraktyki} ECTS (masz ${details.sumPr})` },
      ...
      { ok: checks.ANG,      text: `Język obcy (ANGIELSKI): obecny` }
    ];

    rows.forEach(r => {
      const div = document.createElement("div");
      div.className = `check ${r.ok ? 'ok' : 'bad'}`;
      div.innerHTML = `<span class="dot"></span><span class="text">${r.text}</span>`;
      cl.appendChild(div);
    });
  }
  function serialize(){
  ...
  function deserialize(obj){
    ...
    if (!obj || !obj.semesters || !obj.semesters.length){
      addSemester();
      return;
    }
      ...
      semData.courses.forEach(c => {
        ...
        (c.tags || []).forEach(t => {
          if (![...tagsSel.options].some(o=>o.value===t)){
            const opt = document.createElement("option");
            opt.value = t; opt.textContent = t; tagsSel.appendChild(opt);
          }
        });
  ...
  btnSave.addEventListener("click", () => {
    if (isAllInputsEmpty()){
      ...
      downloadJSON("uwr-plan.json", data);
    }
  });
